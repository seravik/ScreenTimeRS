...
- Windows x64 GitHub Actions build and Inno Setup installer updated for v0.2.3.
...
- The status InfoBar can now be closed with its X button.
- Tray exit requests now close the UI and stop the collector cleanly.
- Responsive overview cards adapt to smaller window widths so usage times remain fully visible.
- Settings content remains left-aligned instead of shifting toward the center when the window is resized.
