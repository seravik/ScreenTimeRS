...
- 优化时间条可悬浮区域，确保鼠标移入每日时间轴即可查看具体使用时长。

### 新增
- “概览 → 最近 14 天”重做为极简 14 日时间轴，鼠标悬浮每日时间条可查看具体使用时长。
- 支持 Windows 登录后后台静默启动，启动时不自动打开主窗口。

### 优化
- 优化 WinUI 页面刷新机制，实时更新只刷新当前显示页面，减少不必要的界面重绘与资源占用。
- 优化 Rust 后台采集器初始化与系统资源采集，降低后台常驻内存与 CPU 开销。
- 优化发布配置，减少不必要的调试文件与发布产物。
- 优化软件启动与运行时的响应速度。
